...
namespace :mysql do
  ...
  task :build_db do
    %x{ mysqladmin -u#{my_config['username']} -p#{my_config['password']} create #{my_config['database']} } rescue "test db already exists"
  ...
  task :drop_db do
    ...
    %x{ mysqladmin -u#{my_config['username']} -p#{my_config['password']} drop #{my_config['database']} --force}
  end
  ...
  task :drop_test_dbs do
    ...
    databases.each do |database|
      puts "dropping database #{database}"
      %x{ mysqladmin -u#{my_config['username']} -p#{my_config['password']} drop #{database} --force}
    end
  end
